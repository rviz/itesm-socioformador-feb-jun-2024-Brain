...
export default function Evaluation() {
  ...
  const allMyCategoryQuestions = [
    educationQ,
    equilibrioTrabajoVidaQ,
    ingresosQ,
    medioAmbienteQ,
    saludQ,
    satisfaccionQ,
  ...
  useEffect(() => {
    const loadUser = async () => {
      try {
        ...
        const responseC3 = await fetch(
          "/api/categoryQuestions/equilibrioTrabajoVida",
        );
  ...
  const CheckIfComplete = async (categoryName) => {
    ...
    categoryQ.forEach((question) => {
      const isAnswered = answersGot.some(
    ...
    if (questionsResponded < categoryQ.length && questionsResponded >= 1) {
      isComplete = 2;
    }
  ...
  return (
    <div>
      {user != null && canLoad == false ? (
        <p className="text-3xl text-center duration-200 mt-40 mb-44">
        ...
        <div>
          ...
          <div>
            {showedCategory === "Vivienda" ? (
              <CompleteEvaluation
                CheckIfComplete={() => CheckIfComplete(showedCategory)}
                categoryName={showedCategory}
                categoryQuestions={viviendaQ}
              />
            ) : showedCategory === "Satisfacción" ? (
              <CompleteEvaluation
                CheckIfComplete={() => CheckIfComplete(showedCategory)}
                categoryName={showedCategory}
                categoryQuestions={satisfaccionQ}
              />
            ) : showedCategory === "Educación" ? (
              <CompleteEvaluation
                CheckIfComplete={() => CheckIfComplete(showedCategory)}
                categoryName={showedCategory}
                categoryQuestions={educationQ}
              />
            ) : showedCategory === "Medio ambiente" ? (
              <CompleteEvaluation
                CheckIfComplete={() => CheckIfComplete(showedCategory)}
                categoryName={showedCategory}
                categoryQuestions={medioAmbienteQ}
              />
            ) : showedCategory === "Salud" ? (
              <CompleteEvaluation
                CheckIfComplete={() => CheckIfComplete(showedCategory)}
                categoryName={showedCategory}
                categoryQuestions={saludQ}
              />
            ) : showedCategory === "Ingresos" ? (
              <CompleteEvaluation
                CheckIfComplete={() => CheckIfComplete(showedCategory)}
                categoryName={showedCategory}
                categoryQuestions={ingresosQ}
              />
            ) : showedCategory === "Seguridad" ? (
              <CompleteEvaluation
                CheckIfComplete={() => CheckIfComplete(showedCategory)}
                categoryName={showedCategory}
                categoryQuestions={seguridadQ}
              />
            ) : showedCategory === "Equilibrio trabajo-vida" ? (
              <CompleteEvaluation
                CheckIfComplete={() => CheckIfComplete(showedCategory)}
                categoryName={showedCategory}
                categoryQuestions={equilibrioTrabajoVidaQ}
              />
            ) : (
              <div />
            )}
