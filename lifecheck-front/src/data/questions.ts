...
const dbanalyis = drizzle(pool, {
  schema: { user_evaluation_category_analysis },
});
...
export async function getAnalysis() {
  const data = await getSession();
  const analysis =
    await dbanalyis.query.user_evaluation_category_analysis.findMany({
...
export async function addAnswerWithQID(
  aText: string,
  questionId: number,
  createdBy: string,
) {
...
export async function replaceAnswerWithQID(
  aText: string,
  questionId: number,
  createdBy: string,
  userID: string,
  ...
  const updateResult = await db
    ...
    .where(
      and(eq(answer.userId, data.user.sub), eq(answer.questionId, questionId)),
    )
    .returning();
