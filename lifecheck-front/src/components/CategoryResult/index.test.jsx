...
describe("MyComponent", () => {
  test("Renderiza el componente correctamente", () => {
    const { getByText } = render(
      <MyComponent
        cardName="General"
        feedback="1. **Retroalimentación General**: #Esta es una retroalimentación general.#"
      />,
    );
  ...
  test("Muestra correctamente el número y título de la recomendación", () => {
    ...
    const { getByText } = render(
      <MyComponent cardName="Seguridad" feedback={feedback} />,
    );
  ...
  test("Aplica los colores correctos para la tarjeta de Vivienda", () => {
    ...
    const { container } = render(
      <MyComponent cardName="Vivienda" feedback={feedback} />,
    );
    fireEvent.mouseEnter(container.querySelector(".flip-card-front"));
    expect(container.querySelector(".flip-card-back")).toHaveClass(
      "bg-[#ffbf77]",
    );
  });
  ...
  test("Oculta el contenido de la recomendación al retirar el mouse", async () => {
    ...
    const { getByText, queryByText } = render(
      <MyComponent cardName="Salud" feedback={feedback} />,
    );
    fireEvent.mouseEnter(getByText("Consejo de Salud"));
    fireEvent.mouseLeave(getByText("Consejo de Salud"));
    await waitFor(() =>
      expect(queryByText("Come saludable")).not.toBeInTheDocument(),
    );
  });

  test("Renderiza múltiples recomendaciones correctamente", () => {
    const feedback =
      "1. **Consejo Uno**: #Contenido uno.# 2. **Consejo Dos**: #Contenido dos.#";
    const { getByText } = render(
      <MyComponent cardName="General" feedback={feedback} />,
    );
  ...
  test("Los colores de fondo y texto cambian basados en la categoría de la tarjeta", () => {
    ...
    const { container } = render(
      <MyComponent cardName="Medio ambiente" feedback={feedback} />,
    );
  ...
  test("No muestra el contenido de la recomendación sin interacción", () => {
    ...
    const { queryByText } = render(
      <MyComponent cardName="Equilibrio trabajo-vida" feedback={feedback} />,
    );
  ...
  test("Maneja correctamente el texto de recomendación largo", () => {
    ...
    const { getByText } = render(
      <MyComponent cardName="General" feedback={longFeedback} />,
    );
